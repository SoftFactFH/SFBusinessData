...
VALUES ('Test', 'Paul', date('1985-12-09'), 2);

// -- Stmt 2: get inserted autoval
Select t1.seq
from sqlite_sequence t1
where t1.name = 'bdscstmr';
...
VALUES (1, date('2017-03-22'));

// -- Stmt 6: get inserted val
Select "t1"."seq"
from "sqlite_sequence" "t1"
where "t1"."name" = 'bdscstmrorder';

// -- Stmt 7: select without table
Select 1 as bdscstmrid, 'Test' as bdscstmrname, 'Paul' as bdscstmrfirstname, date('1985-12-09') as bdscstmrdateofbirth, 2 as bdscstmrtypeid;

// -- Stmt 8: select in from without table
SELECT t1.* 
FROM (SELECT -1 AS bdscstmrorderid, -1 AS bdscstmrordercstmrid, date('2017-03-22') AS bdscstmrorderdate) t1  
